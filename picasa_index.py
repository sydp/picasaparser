...
def dump_html_table(headerdata, tabledata):
    print "<table border=\"1\">"
    print "<thead>%s</thead>" % "".join( "<td>%s</td>" % header for header in headerdata)
...
def dump_html(filenames, thumb_indexes, imagedatum):
    ...
    for n in range(len(filenames)):
        if filenames[n][1] == -1:
            ...
            print "<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><img src=\"thumbs_0.db_%s.jpg\"></td><td><img src=\"thumbs2_0.db_%s.jpg\"></td><td>%s</td><td>%s</td><td>%s</td></tr>" % (n, filenames[n][0], filenames[n][1], bigthumbs, n, n, preview, imagedatum["Original Height"][n], imagedatum["Original Width"][n])
    print "</table></body></html>"


filenames = load_filenames("thumbindex.db" if len(sys.argv) == 1 else sys.argv[1])
headers = ["Name", "Parent"]
dump_html_table(headers, filenames)
sys.exit(0)
